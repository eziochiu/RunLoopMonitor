...
+ (NSString *)backtraceOfAllThread {
    ...
    for (int idx = 0; idx < thread_count; idx++) {
        [result appendString: _backtraceOfThread(threads[idx])];
    }
    return result.copy;
}
...
+ (NSString *)backtraceOfNSThread:(NSThread *)thread {
    return _backtraceOfThread(machThreadFromNSThread(thread));
}
...
NSString * _backtraceOfThread(thread_t thread) {
...
void symbolicate(const uintptr_t * const backtraceBuffer, Dl_info * const symbolsBuffer, const int numEntries, const int skippedEntries) {
    ...
    if (!skippedEntries && idx < numEntries) {
        ec_dladdr(backtraceBuffer[idx], &symbolsBuffer[idx]);
        idx++;
    }
    
    for (; idx < numEntries; idx++) {
        ec_dladdr(CALL_INSTRUCTION_FROM_RETURN_ADDRESS(backtraceBuffer[idx]), &symbolsBuffer[idx]);
    }
}

bool ec_dladdr(const uintptr_t address, Dl_info * const info) {
    info->dli_fname = NULL;
    info->dli_fbase = NULL;
    info->dli_sname = NULL;
    info->dli_saddr = NULL;
